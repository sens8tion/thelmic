...
def same(ch, t, slot, notes, length):
    ...
    have = sorted((int(n["pitch"]), round(float(n["start_time"]), 3), round(float(n["duration"]), 3),
                   int(round(float(n["velocity"]))), round(float(n.get("probability", 1.0)), 2)) for n in got)
    return have == sorted((p, round(s, 3), round(d, 3), v, 1.0) for p, s, d, v in notes)
    ...
    try:
        ...
        for row, (scene, brk, (bass_from, bass_key)) in enumerate(ROWS):
            ...
            lanes = [("SPINE-TINGLER", *SPINES[row], 16.0), (brk, *CHOPS[row], 16.0),
                     ...
                     ("F-HOLE", bass["name"] or "trapdoor", plain(bass), float(bass["length"]))]
            written = []
            for track, name, notes, length in lanes:
                t = index_of(ch, track)
                if not same(ch, t, row, notes, length):
                    write_clip(ch, t, row, name, notes, length)
